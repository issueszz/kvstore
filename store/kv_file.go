...
func NewKvFile(path string, fid uint32, method FileRwMethod) (*KvFile, error) {
...
// Build 加载数据文件信息
func Build(path string, method FileRwMethod) (map[uint32]*KvFile, uint32, error)  {
	...
	if len(FileIds) > 0 {
		...
		for i := 0; i < len(FileIds)-1; i++ {
			id := uint32(FileIds[i])
			file, err := NewKvFile(path, id, method)
			if err != nil {
